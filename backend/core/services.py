...
async def create_compra(command: types.CompraCommand) -> dict:
    engine = get_engine()
    issued_at = _now()
    retention_days = settings.audit_retention_days if settings.audit_retention_days else 365
    expiry = issued_at + datetime.timedelta(days=retention_days)
...
async def validate_qr(qr_payload_in: str) -> Optional[dict]:
    ...
    async with engine.begin() as conn:
        ...
        await conn.execute(
            update(TicketQr)
            .where(TicketQr.id == ticket["id"])
            .values(usado=True, validado_en=_now())
        )
        ticket["usado"] = True
...
async def create_sorteo(command: types.SorteoCommand) -> dict:
    engine = get_engine()
    sorteo_id = secrets.token_hex(16)
    now = _now()
    async with engine.begin() as conn:
        await conn.execute(
            insert(Sorteo).values(
                id=sorteo_id,
