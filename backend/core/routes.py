...
@router.post("/operadores", response_model=schemas.OperadorOut)
async def crear_operador(operador_in: schemas.OperadorIn):
    raise NotImplementedError()


@router.post("/puestos", response_model=schemas.PuestoOut)
async def crear_puesto(puesto_in: schemas.PuestoIn):
    raise NotImplementedError()


@router.post("/compras", response_model=schemas.CompraOut)
async def crear_compra(command: schemas.CompraIn):
    compra = await services.create_compra(services.types.CompraCommand(**command.model_dump()))
...
async def crear_sorteo(sorteo_in: schemas.SorteoIn):
    sorteo = await services.create_sorteo(services.types.SorteoCommand(**sorteo_in.model_dump()))
